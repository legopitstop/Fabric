...
public class RecipeGenerator extends FabricRecipeProvider {
    ...
    public void generate(RecipeExporter exporter) {
        ...
        Item soulJackOLantern = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, "soul_jack_o_lantern"));

        for (int i = 0; i < MorePumpkins.pumpkins.size(); i++) {
            String prefix = (String)MorePumpkins.pumpkins.get(i);
            Item carvedPumpkin = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix+"_carved_pumpkin"));
            Item jack = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix+"_jack_o_lantern"));
            Item soulJack = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix+"_soul_jack_o_lantern"));
            ...
            // Stonecutter
            Item carved = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix+"_pumpkin"));
            // vanilla -> custom
        ...
        for (int i = 0; i < MorePumpkins.pumpkins.size(); i++) {
            ...
            for (int j = 0; j < MorePumpkins.pumpkins.size(); j++) {
                ...
                if (!Objects.equals(prefix1, prefix2)) { // Ignore duplicate input=outputs
                    Item jackOutput = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix1+"_jack_o_lantern"));
                    Item jackInput = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix2+"_jack_o_lantern"));
                    FabricRecipeProvider.offerStonecuttingRecipe(exporter, RecipeCategory.BUILDING_BLOCKS, jackOutput, jackInput);

                    Item soulJackOutput = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix1+"_soul_jack_o_lantern"));
                    Item soulJackInput = Registries.ITEM.get(new Identifier(MorePumpkins.MOD_ID, prefix2+"_soul_jack_o_lantern"));
